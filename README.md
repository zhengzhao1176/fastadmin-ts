...
│   └── lang/                 多语言包（zh-cn / en）
├── tests/                    黑盒 + 单元测试套件
├── doc/                      FastAdmin 官方开发文档镜像（68 篇，离线可读）
├── docker/                   测试栈 docker-compose（MySQL / Redis / PHP / MailHog）
├── scripts/                  数据库重置、种子、冒烟脚本
└── .github/workflows/        CI（基线测试）
```

> `fastAdmin/`（PHP 原版，作为测试对照基准）体积大且为第三方 GPL 代码，**未纳入本仓库** —— 运行完整对照测试时需另行获取，详见下文。
...
`doc/` 目录是 **FastAdmin 官方开发文档的离线镜像**（共 68 篇 HTML），覆盖安装、模块、CRUD、组件、表格、插件、辅助类等全部主题。本地查阅：
...
- `doc/` 为 FastAdmin 官方文档镜像，版权归 FastAdmin / 深圳极速创想科技有限公司所有；仅作离线开发参考，请勿公开转载。
